...
namespace EFC4RESTAPI.Services
{
    ...
    public static class EFCService
    {
        ...
        private static string GetTitle(this EditType type) => type == EditType.Add ? "新增" : (type == EditType.Modify ? "更新" : "删除");
                                                                        ...
                                                                        IEnumerable<Tuple<Guid, T>> ids_entities = null) where T : ISuper
        {
            ...
            try
            {
                ...
                switch (type)
                {
                    ...
                    case EditType.Modify:
                        ...
                        break;
                    case EditType.Remove: es = sets.GetList(i => ids.Contains(i.Id)).ToList(); break;
                }
            ...
            // catch (Exception e) { return await trans.TransactionAsync(false, e.InnerException.Message.ToString()); }
